...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-card/90 backdrop-blur-2xl border-white/10 shadow-glass transition ease-premium data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
  ...
  ({ side = 'left', className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1.5 text-muted-foreground opacity-70 transition-opacity hover:opacity-100 hover:bg-white/10 focus:outline-none">
          <X className="h-4 w-4" />
